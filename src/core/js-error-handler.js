...
function overWriteConsoleError(tracker) {
  ...
  console.error = function (error) {
    ...
    if (!errorObj) {
      ...
      errorLog = new Log(tracker.trackerOptions, 'Error Log', errorLog, tracker.trackerOptions.customizeErrorLog(error))
      tracker.logList.push(errorLog)

      // console.log('Error Traking Log:\n', errorLog)
    } else {
      let errorLog = {
        errorType: 'js',
        errorMsg: errorMsg,
...
function handleErrorEvent (tracker) {
  ...
  window.onerror = function (errorMsg, url, lineNumber, columnNumber, errorObj) {
    ...
    errorLog = new Log(tracker.trackerOptions, 'Error Log', errorLog, tracker.trackerOptions.customizeErrorLog(errorMsg))
    tracker.logList.push(errorLog)
